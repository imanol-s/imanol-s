...
summary: Relational database system designed around normalization constraints — modeled many-to-many relationships across a structured catalog and built circulation tracking on a properly normalized schema.
tags:
  - Python
  - SQL
  - SQLAlchemy
...
Normalizing to 3NF moved publisher and author details out of the book rows — attributes that had been repeated across all 10k+ book records — into one row per distinct entity, so a change updates a single row instead of thousands. The source catalog also contained multiple near-duplicate publisher names; consolidating them behind a single Publishers table removed that inconsistency at the schema level. UML diagrams documented the schema and relationships for team alignment and future development.
